...
async function generateFeature() {
  ...
  if (data.addNavigation) {
    ...
    if (fs.existsSync(sidebarPath)) {
      ...
      if (!sidebarContent.includes(iconImport)) {
  ...
  // ${PascalFeatureName} feature (generated by mongonext-generator)
  {
    text: '${data.properFeatureNamePlural}',
    icon: <${data.icon}Icon />,
    path: '/admin/${featureNamePlural}',
    children: [
      { text: 'All ${data.properFeatureNamePlural}', path: '/admin/${featureNamePlural}' },
      { text: 'Create ${data.properFeatureName}', path: '/admin/${featureNamePlural}/create' },
    ]
  }`;
      
      // Find the navigationItems array and add the new item
      const navigationItemsPattern = /(const navigationItems = \[[\s\S]*?)(\n\])/;
      if (sidebarContent.match(navigationItemsPattern)) {
        sidebarContent = sidebarContent.replace(
